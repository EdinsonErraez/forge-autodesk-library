  ...
  get(oauth2client, originCredentials, parameters = {}) {
    return new Promise(async (resolve, reject) => {
      try {
        ...
        const projects = await httpClient(options);
        const result = { projects }
        if (wasUpdated) result.credentials = credentials;
        if (projects.length === queryStringParameters.limit) {
  ...
  getAll(oauth2client, originCredentials, parameters = {}, originWasUpdated = false) {
    return new Promise(async (resolve, reject) => {
      try {
        ...
        const projects = await this.get(oauth2client, credentials, queryStringParameters);
        if (projects.credentials) {
          // eslint-disable-next-line prefer-destructuring
          credentials = projects.credentials;
          wasUpdated = true;
        }
        ...
        const responseRecursive = await this.getAll(oauth2client, credentials, projects.nextParameters, wasUpdated);
        result = {
          projects: projects.projects.concat(responseRecursive.projects),
        }
  ...
  getById(oauth2client, originCredentials, projectId) {
    return new Promise(async (resolve, reject) => {
      try {
        ...
        const project = await httpClient(options)
